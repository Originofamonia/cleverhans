    ...
    def fprop(self, x, **kwargs):
        ...
        with tf.compat.v1.variable_scope(self.scope, reuse=tf.compat.v1.AUTO_REUSE):
            ...
            for scale in range(log_resolution - 2):
                y = tf.keras.layers.Conv2D(y, self.nb_filters << scale, **conv_args)
                y = tf.keras.layers.Conv2D(y, self.nb_filters << (scale + 1), **conv_args)
                y = tf.keras.layers.AveragePooling2D(y, 2, 2)
